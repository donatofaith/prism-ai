  ...
  try {
    ...
    if (!project) {
      return NextResponse.json(
        {
          error:
            "A project name or symbol is required.",
        },
        {
          status: 400,
        }
    ...
    return NextResponse.json({
        ...
        accounts.map(
          (account) => ({
            ...
            /*
              ...
              Existing frontend code may still
              read walletType until we migrate it.
            */
            walletType:
              account.walletType,
      ...
      /*
        ...
        This prevents the current frontend
        from breaking before we migrate
        page.tsx from wallets -> accounts.
      */
      wallets:
        accounts.map(
          (account) => ({
    ...
    console.error(
      "PRISM project account API error:",
      error
    );
