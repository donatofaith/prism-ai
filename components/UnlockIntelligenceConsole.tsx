...
function allocationLabel(value: AllocationType) {
...
function impactLabel(value?: "large" | "meaningful" | "limited" | "unknown") {
...
function SourceChecks({ sources }: { sources: SourceCheck[] }) {
  if (sources.length === 0) return null;

  return (
    <div className="rounded-2xl border border-[#E4EAF0] p-4 dark:border-[#2A3A4E]">
      <p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#8A99AB] dark:text-[#8294A8]">
        Sources PRISM checked
      </p>
      <div className="mt-3 flex flex-wrap gap-2">
        {sources.map((source) => {
          const matched = source.status === "matched";
          const unavailable = source.status === "unavailable";
          return (
            <span
              key={`${source.name}-${source.role}`}
              title={source.role}
              className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1.5 text-[10px] font-semibold ${
                matched
                  ? "border-[#BFE9E3] bg-[#F0FBF8] text-[#137E72] dark:border-[#28564F] dark:bg-[#102622] dark:text-[#7ED9CC]"
                  : unavailable
                  ? "border-[#E8D7D7] bg-[#FFF8F8] text-[#9A6666] dark:border-[#583636] dark:bg-[#251719] dark:text-[#D6A1A1]"
                  : "border-[#DFE5EC] bg-[#F8FAFC] text-[#69788A] dark:border-[#2D3E52] dark:bg-[#132132] dark:text-[#A9B7C8]"
              }`}
            >
              <span className="h-1.5 w-1.5 rounded-full bg-current opacity-70" />
              {source.name}
              <span className="opacity-60">· {matched ? "matched" : unavailable ? "unavailable" : "checked"}</span>
            </span>
          );
        })}
      </div>
          ...
          <div className="border-b border-[#E8EDF2] p-5 dark:border-[#26364A] sm:p-6 lg:border-b-0 lg:border-r lg:p-7">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#5A6FFF]">Supply schedule</p>
            ...
            {market && (
              <div className="mt-5 inline-flex max-w-full items-center gap-2 rounded-full border border-[#E1E7ED] bg-[#F8FAFC] px-3 py-2 text-xs text-[#526176] dark:border-[#2B3B4E] dark:bg-[#132132] dark:text-[#B8C5D3]">
                ...
                <span className="truncate">{market.name} ({market.symbol.toUpperCase()})</span>
              </div>
            )}
            ...
            >
              {loading ? "Checking schedule..." : data ? "Refresh Unlock Intelligence" : "Run Unlock Intelligence"}
            </button>
            ...
            <div className="mt-5 rounded-2xl bg-[#F7F9FC] p-4 dark:bg-[#0A1522]">
              ...
              <div className="mt-3 space-y-2 text-xs leading-5 text-[#5F6F83] dark:text-[#A9B7C8]">
                ...
                <p>• If no date is verified, what supply context is still available?</p>
              </div>
            </div>
          </div>
            ...
            {loading ? (
              <div className="flex min-h-[260px] items-center justify-center rounded-[18px] bg-[#F7F9FC] dark:bg-[#0A1522]">
                <div className="px-5 text-center">
                  ...
                  <p className="mt-4 text-sm font-semibold text-[#0D1726] dark:text-white">Checking unlock evidence</p>
                  <p className="mt-2 text-xs text-[#78879A] dark:text-[#94A5B8]">
                    PRISM is checking dated schedules, CoinMarketCap supply context, market data, and verified project information.
                  </p>
                </div>
              </div>
            ) : error ? (
              <div className="rounded-[18px] border border-[#F0D0D0] bg-[#FFF7F7] p-5 dark:border-[#693838] dark:bg-[#301D22] sm:p-6">
                <p className="font-semibold text-[#A83D3D] dark:text-[#FFB0B0]">Unlock Intelligence could not run</p>
              ...
              <div className="space-y-4">
                <div className="rounded-[18px] bg-gradient-to-br from-[#F5F7FF] via-[#FBFCFF] to-[#F0FAF8] p-4 dark:from-[#17223A] dark:via-[#101C2B] dark:to-[#102B2B] sm:p-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      <p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#7A899B] dark:text-[#93A3B5]">Next scheduled release</p>
                      <p className="mt-1.5 text-xl font-bold tracking-[-0.03em] text-[#0D1726] dark:text-white sm:text-2xl">{formatDate(nextUnlock.date)}</p>
                      <p className="mt-1 text-xs leading-5 text-[#69788A] dark:text-[#A9B7C8]">
                        {nextUnlock.daysUntil === 0 ? "Scheduled for today" : `${nextUnlock.daysUntil} day${nextUnlock.daysUntil === 1 ? "" : "s"} away`}
                      </p>
                    </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2.5 xl:grid-cols-4">
                    <Metric label={`Tokens${market?.symbol ? ` · ${market.symbol.toUpperCase()}` : ""}`} value={formatAmount(nextUnlock.amount)} />
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-[#E4EAF0] p-4 dark:border-[#2A3A4E]">
                    <p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#8A99AB]">Who is this linked to?</p>
                    <p className="mt-2 text-sm font-semibold leading-6 text-[#172235] dark:text-white">{nextUnlock.allocation}</p>
                    <p className="mt-1 text-xs text-[#718095] dark:text-[#9CACBE]">{allocationLabel(nextUnlock.allocationType)}</p>
                  </div>

                  <div className="rounded-2xl border border-[#E4EAF0] p-4 dark:border-[#2A3A4E]">
                    <p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#8A99AB]">Schedule source</p>
                    <p className="mt-2 text-sm font-semibold leading-6 text-[#172235] dark:text-white">
                      {data.coverage === "modeled" ? "Published project tokenomics" : data.provider}
                    </p>
                    <p className="mt-1 text-xs leading-5 text-[#718095] dark:text-[#9CACBE]">
                      {data.coverage === "modeled"
                        ? "PRISM modeled only fixed vesting terms described by the project. Variable or discretionary distributions are excluded."
                        : "PRISM matched this token to a dated scheduled-release record."}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid gap-3 lg:grid-cols-2">
                  <div className="rounded-2xl border border-[#DDE7F4] bg-[#F7FAFF] p-4 dark:border-[#2B3D55] dark:bg-[#101D2D]">
                    <p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#5A6FFF]">Beginner summary</p>
                    <p className="mt-2 text-sm leading-6 text-[#405064] dark:text-[#C3CEDA]">{beginnerSummary}</p>
                  </div>
                  <div className="rounded-2xl border border-[#DCEBE7] bg-[#F4FBF9] p-4 dark:border-[#27463F] dark:bg-[#102521]">
                    <p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#159786]">Why it may matter</p>
                    <p className="mt-2 text-sm leading-6 text-[#405064] dark:text-[#C3CEDA]">{watchText}</p>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-[#E4EAF0] dark:border-[#2A3A4E]">
                  <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3.5">
                    <div>
                      <p className="text-sm font-semibold text-[#172235] dark:text-white">Upcoming schedule</p>
                  ...
                  <div className="max-h-[360px] overflow-y-auto border-t border-[#E8EDF2] dark:border-[#26364A]">
                    {visibleEvents.map((event) => (
                      ...
                      >
                        <span className="font-semibold text-[#172235] dark:text-white">{formatDate(event.date)}</span>
                        <span className="min-w-0 break-words text-[#66768A] dark:text-[#A9B7C8]">{event.allocation}</span>
                ...
                <SourceChecks sources={data.sourcesChecked ?? []} />
              ...
              <div className="space-y-4">
                <div className="rounded-[18px] border border-[#DDE4E9] bg-[#F8FAFC] p-5 dark:border-[#2B3A4D] dark:bg-[#0A1522] sm:p-6">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-[#69788A] shadow-sm dark:bg-[#152538] dark:text-[#A9B7C8]">◇</div>
                  <p className="mt-4 text-base font-semibold text-[#0D1726] dark:text-white">
                    {data.supplyContext
                      ? "No dated unlock schedule was verified."
                      : data.coverage === "provider_unavailable"
                      ? "Schedule sources are temporarily unavailable."
                      : "No verified dated unlock schedule found."}
                  </p>
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-[#69788A] dark:text-[#A9B7C8]">
                    {data.message ?? "PRISM checked the available evidence but could not verify a future dated unlock for this token."}
                  </p>

                  {data.supplyContext && (
                    <>
                      <div className="mt-5 rounded-2xl border border-[#DDE7F4] bg-white p-4 dark:border-[#2B3D55] dark:bg-[#101D2D]">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            <p className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#5A6FFF]">Current supply context</p>
                            <p className="mt-1 text-sm font-semibold text-[#172235] dark:text-white">Verified token identity and supply snapshot</p>
                          </div>
                          <span className="rounded-full bg-[#EEF2FF] px-2.5 py-1 text-[10px] font-semibold text-[#5368E8] dark:bg-[#1A2850] dark:text-[#AEB9FF]">CoinMarketCap</span>
                        </div>

                        <div className="mt-4 grid grid-cols-2 gap-2.5 xl:grid-cols-4">
                          <Metric label="Circulating supply" value={`${formatAmount(data.supplyContext.circulatingSupply)} ${data.supplyContext.symbol.toUpperCase()}`} />
                          <Metric label="Total supply" value={`${formatAmount(data.supplyContext.totalSupply)} ${data.supplyContext.symbol.toUpperCase()}`} />
                          <Metric label="Currently circulating" value={formatPercent(data.supplyContext.percentCirculatingOfTotal)} />
                          <Metric label="Fully diluted value" value={formatUsd(data.supplyContext.fullyDilutedMarketCap)} />
                        </div>

                        <p className="mt-4 text-xs leading-5 text-[#69788A] dark:text-[#A9B7C8]">
                          This supply snapshot is useful context, but it is not a dated vesting calendar. PRISM will not turn the gap between circulating and total supply into an unlock date without schedule evidence.
                        </p>
                      </div>
                    </>
                  )}
              ...
              <div className="flex min-h-[260px] items-center justify-center rounded-[18px] border border-dashed border-[#D8E0E8] bg-[#FBFCFD] p-6 text-center dark:border-[#2A3B4F] dark:bg-[#0A1522]">
                <div className="max-w-sm">
                  <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-xl bg-[#EEF1FF] text-[#465FFF] dark:bg-[#1B2850] dark:text-[#AAB5FF]">↗</div>
                  <p className="mt-4 text-sm font-semibold text-[#172235] dark:text-white">Run Unlock Intelligence for the scanned token</p>
                  <p className="mt-2 text-xs leading-5 text-[#7B899A] dark:text-[#8FA0B4]">
                    PRISM checks dated unlock schedules first, then adds CoinMarketCap and CoinGecko supply/market context when a dated schedule cannot be verified.
                  </p>
                </div>
              </div>
            )}
