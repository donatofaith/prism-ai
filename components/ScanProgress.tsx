...
const stages = [
  {
    ...
    description: "Finding the correct asset and market identity.",
  },
  {
    key: "market",
    label: "Loading market",
    description: "Reading price, volume and market-cap activity.",
  },
  {
    key: "accounts",
    label: "Checking attribution",
    description: "Looking for evidence-backed project accounts.",
  },
  {
    key: "history",
    label: "Searching history",
    description: "Comparing the current move with previous periods.",
  },
] as const;
...
export default function ScanProgress({ stage }: ScanProgressProps) {
  ...
  const complete = stage === "complete" || stage === "perspective";
  const progress = complete
    ? 100
    : Math.max(
        8,
        Math.min(92, ((currentIndex - 1) / stages.length) * 100)
      );
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-[#0D1726]">
              {complete ? "Investigation complete" : "Building your intelligence view"}
            </p>
          </div>
          ...
          >
            <span
              className={`h-2 w-2 rounded-full ${
                complete ? "bg-[#16B8A6]" : "animate-pulse bg-[#465FFF]"
              }`}
            />
      ...
      <div className="p-3 sm:p-4">
        <div className="space-y-1.5">
          {stages.map((item) => {
            ...
            const done = complete || currentIndex > itemIndex;
            const active = !complete && stage === item.key;
                ...
                <div className="min-w-0">
                  <p
                    className={`text-xs font-semibold transition ${
                      active || done ? "text-[#0D1726]" : "text-[#98A5B5]"
                    }`}
                  >
                    {item.label}
                  </p>
                  <p
                    className={`mt-1 text-[11px] leading-5 transition ${
                      active ? "text-[#69788A]" : "text-[#B1BBC7]"
                    }`}
                  >
                    {item.description}
                  </p>
